...
calculate_hours <- function(object = object){
  message("[EcoToxR]:  Recalculate duration data (day to hours).")
...
calculate_water_solubility <- function(object = object){
  ...
  pb <- progress::progress_bar$new(
    format = "[EcoToxR]:  Estimate the solubility domain [:bar] :percent ETA: :eta",
...
export_chemical_list <- function(object,project_path){
  message("[EcoToxR]:  The list of included chemicals is exported to the project folder.")
...
export_exclude_list <- function(object,project_path){
  message("[EcoToxR]:  The final list is exported to the project folder.")
...
update_chemical_list <- function(object = object, project_path = project_path, database_path = database_path){
  ...
  length_progressbar <- length(col_names1)
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Update the chemical properties [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
...
create_chemical_properties <- function(database_path){
  if (file.exists(file.path(database_path,"chemical_properties.csv"))){
    message("[EcoToxR]:  Read chemical properties (this is a custom file).")
...
export_mol_units <- function(object, project_path = project$project_path){
  ...
  message("[EcoToxR]:  The list of chemicals for mol/L to mg/L conversion is exported to the project folder.")
...
update_mol_units <- function(object, database_path = project$database_path, project_path = project$project_path){
  message("[EcoToxR]:  Record unit mol/L are recalculated to mg/L.")
  ecotoxgroup <- object$parameters$ecotoxgroup
...
convert_units <- function(object, sample_size = NA){
  ...
  # ng related
  message("[EcoToxR]:  Convert units to human readible format (i.e. mg/L).")
  ...
  if(length_progressbar > 0){
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Convert pg/L like units [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
    ...
    pb$terminate()
  } else {message("[EcoToxR]:  Skip pg/L like unit conversion")}
  ...
  if(length_progressbar > 0){
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Convert ng/L like units [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
    ...
    pb$terminate()
  } else {message("[EcoToxR]:  Skip ng/L like unit conversion")}
  ...
  if(length_progressbar > 0){
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Convert ug/L like units [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
    ...
    pb$terminate()
  } else {message("[EcoToxR]:  Skip ug/L like units conversion")}
  ...
  if(length_progressbar > 0){
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Converte g/L like units [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
    ...
    pb$terminate()
  } else {message("[EcoToxR]:  Skip g/L like units conversion")}
  
  
  
  message("[EcoToxR]:  Convert mg/L like units")
  ...
  if(length_progressbar > 0){
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Convert pmol/L like units [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
    ...
    pb$terminate()
  } else {message("[EcoToxR]:  Skip pmol/L like unit conversion")}
  ...
  if(length_progressbar > 0){
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Convert nmol/L like units [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
    ...
    pb$terminate()
  } else {message("[EcoToxR]:  Skip nmol/L coversion")}
  ...
  if(length_progressbar > 0){
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Covert umol/L like units [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
    ...
    pb$terminate()
  } else {message("[EcoToxR]:  Skip umol/L like unit conversion")}
  ...
  if(length_progressbar > 0){
    pb <- progress::progress_bar$new(
      format = "[EcoToxR]:  Convert mmol/L like units [:bar] :percent ETA: :eta",
      total = length_progressbar, clear = FALSE, width = 80)
    ...
    pb$terminate()
  } else {message("[EcoToxR]:  Skip mmol/L like unit conversion")}
...
save_project <- function(object = object, project_path = project_path, save_project_steps = save_project_steps){
  ...
  if(isTRUE(save_project_steps)){
    project <- object
    message("[EcoToxR]:  Save the project state.")
