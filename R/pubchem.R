...
for (i in 1:nrow(chemical_list_no_SMILES)){
    ...
    if (is.na(pc_props$CanonicalSMILES)) {
        ...
        pubchem_new_row <- pubchem_new_row %>% add_row() %>% mutate(cas_number = cas_numb, cas = casrn,
        ...
        if (!is_empty(which(names(pc_props) %like% "IUPACName"))) {
            ...
            pc_props <- pc_props %>% select(cas_number, cas, cid, PREFERRED_NAME, CASRN,
        ...
        pc_props <- pc_props %>% add_column(FOUND_BY = "CASRN in PubChem", .before = 3)
...
chemicals_update <- chemicals_update %>%
    mutate(CID = pubchem$cid, FOUND_BY = pubchem$FOUND_BY, PREFERRED_NAME = pubchem$PREFERRED_NAME,
           CASRN = pubchem$CASRN, INCHIKEY = pubchem$INCHIKEY, IUPAC_NAME = pubchem$IUPAC_NAME,
           SMILES = pubchem$SMILES, INCHI_STRING = pubchem$INCHI_STRING, MOLECULAR_FORMULA = pubchem$MOLECULAR_FORMULA,
