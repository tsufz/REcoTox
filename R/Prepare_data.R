prepare_data <- function(project, effects = c("MOR","GRO","DEV"),
  ...
  if (load_initial_project == TRUE) {
    ...
    if (file.exists(initial_project)) {
      message("[EcoToxR]:  Load the initial project.")
      .tempenv <- new.env()
  ...
  message("[EcoToxR]:  Subset to habitat.")
  object$tests_habitat <- object$tests[organism_habitat %in% habitat]
  
  # Merge the raw tables
  message("[EcoToxR]:  Merge tests, results and chemicals.")
  ...
  # clean up data
  message("[EcoToxR]:  Clean up effects, endpoints and measurements of asterics. tilde and slash artefacts.")
  ...
  message(paste0("[EcoToxR]:  Shrink to effects: ", paste(effects, collapse = ", "), "."))
  object$parameters$effects <- effects
  ...
  if (remove_formulation == TRUE) {
    message("[EcoToxR]:  Remove formulations.")
  ...
  message("[EcoToxR]:  Reduce to mass/water related units.")
  ...
  if (save_project == TRUE) {
    message("[EcoToxR]:  Save the initial project to the project folder.")
